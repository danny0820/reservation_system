...
#### `UserSearchResponse` (搜尋回應)
- **用途**: 用戶搜尋結果的資料格式
- **功能**: 包含匹配的用戶資訊

#### `UserFilterRequest` (篩選請求)
- **用途**: 多條件篩選的請求參數
- **欄位**: `role`, `status`, `created_after`, `created_before`, `skip`, `limit`

#### `UserStatsResponse` (統計回應)
- **用途**: 用戶統計資訊的回應格式
- **功能**: 提供用戶數量統計
...
- `search_users(db, query, skip, limit)`: 關鍵字搜尋用戶
- `filter_users(db, filter_request)`: 多條件篩選用戶
- `get_user_stats(db)`: 獲取用戶統計資訊
...
- `GET /stylists`: 獲取所有設計師
- `GET /customers`: 獲取所有客戶

#### 用戶搜尋與篩選 API（已認證用戶可用）
- `GET /search?q={keyword}`: 關鍵字搜尋用戶
- `GET /filter`: 多條件篩選用戶
...
- **資料庫模型**: 1 個主要模型 + 1 個枚舉
- **資料驗證模型**: 14 個 Pydantic 模型
- **CRUD 方法**: 19 個資料庫操作方法
- **API 端點**: 25 個 REST API 端點
- **認證函數**: 7 個認證相關函數
- **配置項目**: 13 個應用程式設定
...
*最後更新: 2025-07-16 - 新增權限分層和擴展 API 端點*
*維護者: 開發團隊*
